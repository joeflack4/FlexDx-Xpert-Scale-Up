...
/*        var n_colours = ['#fb9902','#fd5308','#fe2712','#a7194b','#8601af',
        ...
        t_str += "<tr><th></th><td>Baseline Smear, No Xpert</td>";
        if (order_by != 0) {
            t_str += "<td>";
            t_str += (round_to(baseline['cost_val']*-1,0)).toString() + "%";
            t_str += "</td>";
        } else {
            t_str += "<td>0%</td>";
        }
        ...
        if (order_by != 0) {
            t_str += "<td>";
            t_str += (round_to(baseline['type_val']*-1,0)).toString() + "%";
            t_str += "</td>";
        } else {
            t_str += "<td>0%</td>";
        }
        t_str += "<td></td>";
        t_str += "<td><input id='rb_";
        t_str += (type === 'tbi') ? 'tbi' : 'mdr';
        t_str += "_0' type='radio' name='rgb_";
        t_str += (type === 'tbi') ? 'tbi' : 'mdr';
        t_str += "' value='0'";
        if (order_by == 0) {
    ...
    XPT_u.create_comp_table = function ( tb ) {
        ...
        for (var j=0;j<8;j++) {
            ...
            t_string+="<th>"+(j+1).toString()+"</th><th>"+s_a[j]+"</th>";
            for (var x=0;x<5;x++) {
    ...
    $("[name='data_ex']").click ( function () {
        location.replace('/static/excel/excel_' + XPT_u.iso3 + '.xls');
    });
