...
class Algorithm:
    ...
    def getFuns(self):
        ...
        for k in product(range(2),range(2),range(3),range(2)):   # loop over hpdi - d has different meaning
            ...
            pe[h,p,d,i] = emp_tx * (1-p1[h,p,d,i]-p2[h,p,d,i])   # empiric treatment for those not treated
            dlydata[h,p,d,i] = (dly[h,p,i,dd,1] + dly[h,p,i,dd,2] + Te*pe[h,p,d,i])/ (p1[h,p,d,i]+p2[h,p,d,i]+pe[h,p,d,i]+1e-9) # dly include probs, conditioned on treatment
...
def hb_log ( message ):
    with open (prod_dev_path + "homebrew.log", 'a') as fp:
        fp.write( '{} - ({}): {}\n'.format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),ip_addr, message ) )

###############################################################

# read in homebrew form json data
# with open('homebrew_input.json','r') as fd:
"""
...
try:
    ud_strat_name = jdata['ud_strat_name']
except:
...
intopt = 0
int_select = 9
target_inc = float(jdata['model_inputs']['inc']) # 250.
target_mdr = float(jdata['model_inputs']['mdr']) / 100.0 #3.7 / 100
target_hiv = float(jdata['model_inputs']['hiv']) # 0.83
drug1_cost = float(jdata['model_inputs']['drug1_cost']) #500.
drug2_cost = float(jdata['model_inputs']['drug2_cost']) #1000.
drug3_cost = float(jdata['model_inputs']['drug3_cost']) #5000.
outpt_cost = float(jdata['model_inputs']['outpt_cost']) #10.
sm_cost = float(jdata['model_inputs']['sm_cost']) #2.
gxp_cost = float(jdata['model_inputs']['gxp_cost']) #15.
sdgxp_cost = float(jdata['model_inputs']['sdgxp_cost']) - gxp_cost #30. - gxp_cost


outdata = {}

outdata['int_select'] = int_select
outdata['target_inc'] = target_inc
outdata['target_mdr'] = target_mdr * 100
outdata['target_hiv'] = target_hiv
outdata['drug1_cost'] = drug1_cost
outdata['drug2_cost'] = drug2_cost
outdata['drug3_cost'] = drug3_cost
outdata['outpt_cost'] = outpt_cost
outdata['sm_cost'] = sm_cost
outdata['gxp_cost'] = gxp_cost
outdata['sdgxp_cost'] = sdgxp_cost + gxp_cost# - gxp_cost
outdata['progress'] = -1
...
ltfu = 0.15**1              # 15% initial default proportion, smear/GXP
ltfu_both = 0.2          # 25% initial default proportion, smear then GXP
emp_tx = 0.25**6            # 25% of pts treated empirically - SET ZERO
...
t_lpa = 20.0/365         # time to DST using LPA, per Boehme Lancet
lpa_sens = .97           # from Medscape http://www.medscape.com/viewarticle/740253_9
lpa_spec = .99           # from Medscape http://www.medscape.com/viewarticle/740253_9
lpa_cost = 9.4           # from FIND http://www.finddiagnostics.org/about/what_we_do/successes/find-negotiated-prices/mtbdrplus.html
t_rad = 1.0/365                # time to chest X-ray result
rad_sens = 0.978         # X-ray van t'Hoog's review http://www.who.int/tb/Review2Accuracyofscreeningtests.pdf
rad_spec = 0.754         # X-ray van t'Hoog's review http://www.who.int/tb/Review2Accuracyofscreeningtests.pdf
rad_cost = 10            # bit of a guess
t_lam = 1.0/365          #quick
lam_sens = 0.5
lam_spec = .999
lam_cost = 3.5           #http://www.aidsmap.com/Determine-LAM-urine-antigen-TB-test-is-highly-cost-effective-for-use-in-hospitalised-people-living-with-HIV/page/2455261/


dur_fail = 6.            # number of months until failing cases are re-evaluated
prop_cough = 0.01**1        # proportion of people w/ non-TB cough eval'd per yr - SET ZERO

Te = t_smgxp             # empirical tx delay -- added pjd
##################################
...
# basic tests
test_smr = Diagnostic([0.,1.0],sm_spec,(sm_cost+outpt_cost),t_smgxp,ltfu,label='smr',DRsens=0,DRspec=1) # smear as a test
test_sdgxp = Diagnostic([gxp_sens,1.0],gxp_spec,(gxp_cost+outpt_cost+sdgxp_cost),1.0/365,0.0,label='sdgxp',DRsens=gxr_sens,DRspec=gxr_spec) # sd GXP as a test
test_gxp = Diagnostic([gxp_sens,1.0],gxp_spec,(gxp_cost+outpt_cost),t_smgxp,ltfu,label='gxp',DRsens=gxr_sens,DRspec=gxr_spec) # GXP as a test
test_gxpC = Diagnostic([1.0,1.0],0,(gxp_cost),1e-9,0,label='gxpc',DRsens=gxr_sens,DRspec=gxr_spec) # GXP confirmatory
...
def algfromdict(J):
    ...
    testdict = {}
    # define known tests
    testdict['CXR'] = Diagnostic( [rad_sens,rad_sens], rad_spec, rad_cost, t_both, 0, label='Chest X-ray', DRsens=0, DRspec=1 ) # chest X-ray
    testdict['Smear'] = Diagnostic( [0,1], sm_spec, (sm_cost+outpt_cost), t_smgxp, ltfu, label='Smear', DRsens=0, DRspec=1 ) 
    testdict['GXP'] = Diagnostic( [gxp_sens,1], gxp_spec, (gxp_cost+outpt_cost), t_smgxp, ltfu, label='GXP', DRsens=gxr_sens, DRspec=gxr_spec )
    testdict['LAM'] = Diagnostic( [lam_sens,lam_sens], lam_spec, (outpt_cost + lam_cost), t_lam, 0, label='LAM', DRsens=0, DRspec=1 ) 
    testdict['DST_GXP'] = Diagnostic( [1,1], 0, gxp_cost, t_smgxp, 0, label='DST_GXP', DRsens=gxr_sens, DRspec=gxr_spec )
    testdict['DST_LPA'] = Diagnostic( [1,1], 0, lpa_cost, t_lpa, 0, label='DST_LPA', DRsens=lpa_sens, DRspec=lpa_spec )
    ...
    testdict['Culture'] = Diagnostic( [cx_sens,1], cx_spec, (gxp_cost+outpt_cost), t_cxr, ltfu, label='CX', DRsens=0, DRspec=1 )
    testdict['DST_Culture'] = Diagnostic( [1,1], 0, gxp_cost, t_cxr, ltfu, label='DST_CX', DRsens=cxr_sens, DRspec=cxr_spec )
...
if target_inc2 < 50:
    ...
    while equil_round<50:
        ...
        hb_log("{} {} {}".format(equil_round, beta_s, incprevmort(EQUI[50,:])[0] + incprevmort(EQUI[50,:])[1]))
...
if target_mdr2 > target_mdr:
    ...
    while equil_round<50:
        ...
        hb_log("{} {} {}".format(equil_round, fit_m, incprevmort(EQUI2[50,:])[4]/incprevmort(EQUI2[50,:])[0]))
